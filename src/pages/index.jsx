...
import url_suratKeluar from "./api/spreadsheet-suratKeluar";
import url_suratMasuk from "./api/spreadsheet-suratMasuk";
...
export default function Home({ suratMasuk, suratKeluar, countSuratKeluar, countSuratMasuk }) {
  ...
  // google sheets
  const gsJumlahSuratKeluar = countSuratKeluar.length;
    const gsJumlahSuratMasuk = countSuratMasuk.length;


  return (
    <>
      <Head>
        <title>Home-SIA PD IPM Pekanbaru</title>
      </Head>
            ...
            <div className="col">
              <div className="card border-light border-1 d-flex justify-content-center p-4">
                <div className="card-body">
                  ...
                  <h1 id="jumlahsuratmasuk">{gsJumlahSuratMasuk}</h1>
                  <div>
              ...
              <div className="col-md-6 d-flex flex-column justify-content-center">
                <div className="text-white p-4 p-md-5">
                  ...
                  <div className="my-3">
                    ...
                    <Link className="btn btn-light mt-2" role="button" href="#">
                      Button
                    </Link>
                  </div>
                </div>
            ...
            <div className="col">
              <div className="card border-warning border-2 h-100">
                <div className="card-body d-flex flex-column justify-content-between p-4">
                  ...
                  <Link className="btn btn-warning" role="button" href="#">
                    Button
                  </Link>
                </div>
              </div>
...
export async function getServerSideProps() {
  // Fetch data for tb_suratMasuk
  let { data: tb_suratMasuk, error: errorSuratMasuk } = await supabase
    .from("tb_suratMasuk")
    .select("id", { count: "exact" });

  // Fetch data for tb_suratKeluar
  let { data: tb_suratKeluar, error: errorSuratKeluar } = await supabase
    .from("tb_suratKeluar")
    .select("id", { count: "exact" });

  // Fetch data for suratKeluar
  const urlsuratKeluar = url_suratKeluar;
  const resultSuratKeluar = await fetch(urlsuratKeluar);
  const { sheets: sheetsSuratKeluar } = await resultSuratKeluar.json();
  const eventSheetSuratKeluar = sheetsSuratKeluar[0];
  const data_suratKeluar = eventSheetSuratKeluar.data[0].rowData
  ...
  const urlsuratMasuk = url_suratMasuk;
  const resultSuratMasuk = await fetch(urlsuratMasuk);
  const { sheets: sheetsSuratMasuk } = await resultSuratMasuk.json();
  const eventSheetSuratMasuk = sheetsSuratMasuk[0];
  const data_suratMasuk = eventSheetSuratMasuk.data[0].rowData
    .filter((_, index) => index !== 0)
    .map((row) => {
      const { values } = row;
      return {
  ...
  return {
    props: {
      ...
      countSuratKeluar: data_suratKeluar,
      countSuratMasuk: data_suratMasuk,
    },
  };
}
